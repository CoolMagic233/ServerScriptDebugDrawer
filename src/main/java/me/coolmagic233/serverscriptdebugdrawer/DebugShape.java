...
public abstract class DebugShape {
     ...
     */
    public void addViewer(Player player, boolean sendPacketImmediately) {
        if (this.viewers.put(player.getId(), player) == null && sendPacketImmediately) {
            var packet = new ServerScriptDebugDrawerPacket();
            packet.shapes.add(this.toNetworkData());
            player.dataPacket(packet);
        }
    }

    /**
     * @see #removeViewer(Player, boolean)
     */
     ...
     */
    public void removeViewer(Player player, boolean sendPacketImmediately) {
        if (this.viewers.remove(player.getId()) != null && sendPacketImmediately) {
            var packet = new ServerScriptDebugDrawerPacket();
            packet.shapes.add(createRemovalNotice());
            player.dataPacket(packet);
        }
    }
    ...
    protected void onChange() {
        for (Player viewer : this.viewers.values()) {
            var packet = new ServerScriptDebugDrawerPacket();
            packet.shapes.add(this.toNetworkData());
     ...
     */
    public abstract CloudBrustDebugShape.Type getType();
     ...
     */
    public CloudBrustDebugShape createRemovalNotice() {
     ...
     */
    public abstract CloudBrustDebugShape toNetworkData();
}
